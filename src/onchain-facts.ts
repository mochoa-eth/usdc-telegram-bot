...
/** CCTP and Gateway contracts, which share one address across every supported EVM chain. */
...
const BY_CONTRACT_NAME = new Map(EVM_PROTOCOL_CONTRACTS.map((contract) => [contract.name, contract]));
...
export function getProtocolContractByName(name: string): ProtocolContractFact | undefined {
  return BY_CONTRACT_NAME.get(name);
}
